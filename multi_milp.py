...
def get_args():
    ...
    parser.add_argument("--memory_footprint_reduction", type=int, default=5, help='Memory Footprint Reduction Constraint (MiB)')
...
class MILPMultiWorkerFormulation:
    ...
    def prepare(self, memory_footprint_reduction, tensor_size_threshold = 1e5):
        self.p = gp.Model() #MixedIntegerLinearProgram(maximization=False, solver='Gurobi')
        ...
        #variable definition
        # swap_in_start_time 
        self.I = self.p.addMVar((num_gpus, num_kernels + 1), vtype=gp.GRB.SEMICONT, name="I")
        
        # swap_out_start_time
        self.O = self.p.addMVar((num_gpus, num_kernels + 1), vtype=gp.GRB.SEMICONT, name="O")
        
        # kernel_start_time
        self.K = self.p.addMVar((num_gpus, num_kernels + 1), vtype=gp.GRB.SEMICONT, name="K")
        
        # 0 : we do not swap the block, 1 : we swap the block
        self.d = self.p.addMVar((num_gpus, num_tensors), vtype=gp.GRB.BINARY, name="d")
        self.dd = self.p.addMVar((num_tensors), vtype=gp.GRB.BINARY, name="dd")
    ...
    def formulate(self):
        p, I, K, O, d, dd = self.p, self.I, self.K, self.O, self.d, self.dd
        swap_out_end_time = [dict() for gidx in range(self.num_gpus)]

        time_inf = 10 * self.total_duration
        mem_usage = [0 for gidx in range(self.num_gpus)]
        
        tidx_need_dd = set()

        for gidx in range(self.num_gpus):
            for kidx, kernel in self.info.kernels.items():
                ...
                for tidx in kernel.dependency:
                    ...
                    if kidx == self.kidx_range[tidx].end and size > self.tensor_size_threshold:
                        ...
                        tidx_need_dd.add(tidx)
                        dI += d[gidx][tidx] * memory_movement_time + dd[tidx] * memory_movement_time
                        # if not swapped out, memory usage is decreased
                        mem_usage[gidx] -= (1 - d[gidx][tidx]) * self.info.tensor_size[tidx]
                        swap_out_kidx = self.kidx_range[tidx].begin
                        
                        # swap-in begin later than swap-out end time 
                        p.addConstr(I[gidx][kidx] >= swap_out_end_time[gidx][swap_out_kidx] - (1 - d[gidx][tidx]) * time_inf)
                
                p.addConstr(K[gidx, kidx] >= I[gidx, kidx] + dI)

                # we only add memory constraint at peak memory usage
                if kidx == self.peak_kidx:
                    p.addConstr(mem_usage[gidx] <= self.peak_memory_constraint)

                dK = kernel.duration

                # after kernel, we swap out dependencies
                p.addConstr(O[gidx][kidx] >= K[gidx][kidx] + dK)
                ...
                for tidx in kernel.dependency:
                    ...
                    if kidx == self.kidx_range[tidx].begin and size > self.tensor_size_threshold:
                        memory_movement_time = size/h2d_bw 
                        tidx_need_dd.add(tidx)
                        dO += d[gidx][tidx] * memory_movement_time + dd[tidx] * memory_movement_time
                        # if not swapped out, memory usage is increased
                        mem_usage[gidx] += (1 - d[gidx][tidx]) * info.tensor_size[tidx] 

                swap_out_end_time[gidx][kidx] = O[gidx][kidx] + dO

                p.addConstr(I[gidx][kidx + 1] >= I[gidx][kidx] + dI)
                p.addConstr(K[gidx][kidx + 1] >= K[gidx][kidx] + dK)
                p.addConstr(O[gidx][kidx + 1] >= O[gidx][kidx] + dO)

        
        # create dd
        # https://or.stackexchange.com/questions/37/how-to-linearize-the-product-of-two-binary-variables
        # TODO: general multi-workers (>2)
        for tidx in tidx_need_dd:
            p.addConstr(d[0][tidx] >= dd[tidx]) 
            p.addConstr(d[1][tidx] >= dd[tidx]) 
            p.addConstr(dd[tidx] >= d[0][tidx] + d[1][tidx] - 1) 
            
        p.addConstr(K[1][self.info.num_kernels()] >= K[0][self.info.num_kernels()])
        p.setObjective(K[1][self.info.num_kernels()], gp.GRB.MINIMIZE)
    ...
    def summary(self):
        ...
        for rank in range(2):
            selected = [tid for tid, s in schedule[rank].items() if s == 1]
            selected_size = sum([self.info.tensor_size[tid] for tid in selected])
            generate_report(f"Summary Rank-{rank}", {
    ...
    def get_schedule(self):
        ...
        return (to_dict(self.d.X[0]), to_dict(self.d.X[1]))         
    
    def simulate_swapped_trace(self):
        p, I, K, O, d, dd = self.p, self.I, self.K, self.O, self.d, self.dd

        mem_usage = list(0 for gidx in range(self.num_gpus))
        mem_usage_trace = [list() for gidx in range(self.num_gpus)]
        timelines = [defaultdict(list) for gidx in range(self.num_gpus)]
        ...
        for gidx in range(self.num_gpus):
            for kidx, kernel in info.kernels.items():
                ...
                for tidx in kernel.dependency:
                    size = self.info.tensor_size[tidx]
                    if kidx == self.kidx_range[tidx].end and size > self.tensor_size_threshold:
                        decision = p.get_values(d[gidx][tidx])
                        memory_movement_time = size/h2d_bw 
                        dI += decision * memory_movement_time + p.get_values(dd[tidx]) * memory_movement_time
                        # if not swapped out, memory usage is decreased
                        mem_usage[gidx] -= (1 - decision) * info.tensor_size[tidx]

                mem_usage_trace[gidx].append((p.get_values(K[gidx][kidx]), mem_usage[gidx]))
                ...
                for tidx in kernel.dependency:
                    ...
                    if kidx == self.kidx_range[tidx].begin and size > self.tensor_size_threshold:
                        ...
                        dO += decision * memory_movement_time + p.get_values(dd[tidx]) * memory_movement_time
                        # if not swapped out, memory usage is increased
                        mem_usage[gidx] += (1 - decision) * self.info.tensor_size[tidx] 

                timelines[gidx]['swap-in'].append(TimeSegment(p.get_values(I[gidx][kidx]), dI))
                timelines[gidx]['swap-out'].append(TimeSegment(p.get_values(O[gidx][kidx]), dO))                      
                timelines[gidx]['kernel'].append(TimeSegment(p.get_values(K[gidx][kidx]), kernel.duration))
            
            
        ret = [np.array(mem_usage_trace[gidx]).transpose() for gidx in range(self.num_gpus)]
        return ret, timelines
...
if __name__ == '__main__':
    ...
    if args.num_gpu != 2:
        print("Current we only support num_gpu = 2")
        sys.exit(-1)
    ...
    input_path = os.path.join(args.path, f'{args.scheduler_input}.pkl')
    with open(input_path, 'rb') as f:
        print("import from", input_path)
        info = pickle.load(f)

    info.kernels = kernel_filter(info.kernels, kernel_time_threshold)

    milp = MILPMultiWorkerFormulation(info)
    ...
    for rank in range(args.num_gpu):
        output_path = os.path.join(args.path, f'{args.scheduler_output}.rank{rank}.pkl')
        with open(output_path, 'wb') as f:
            print("export to", output_path)
            pickle.dump(schedule[rank], f)
